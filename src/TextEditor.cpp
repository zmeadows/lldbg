...
TextEditor::Coordinates TextEditor::ScreenPosToCoordinates(const ImVec2& aPosition) const
{
    ...
    */
    ...
    float cumulatedStringWidth[2] = {
        0.0f, 0.0f};  //( [0] is the lastest, [1] is the previous. I use that trick to check
                      // where cursor is exactly (important for tabs)
    ...
    if (lineNo >= 0 && lineNo < (int)mLines.size()) {
        ...
        // Then we reduce by 1 column coord if cursor is on the left side of the hovered
        // column.
...
TextEditor::Line& TextEditor::InsertLine(int aIndex)
{
    ...
    for (auto& i : mErrorMarkers)
        etmp.insert(
            ErrorMarkers::value_type(i.first >= aIndex ? i.first + 1 : i.first, i.second));
    mErrorMarkers = std::move(etmp);
    ...
    if (ImGui::IsWindowHovered()) {
        if (!shift && !alt) {
            ...
            */
                ...
                line_clicked_this_frame = mState.mCursorPosition.mLine + 1;
...
void TextEditor::Render()
{
    ...
    // Deduce mTextStart by evaluating mLines size (global lineMax) plus two spaces as text
    // width
    char buf[16];
    ...
    if (!mLines.empty()) {
        ...
        while (lineNo <= lineMax) {
            ...
            // 	if (!HasSelection())
            // 	{
            // 		auto end = ImVec2(start.x + contentSize.x + scrollX, start.y +
            // mCharAdvance.y); 		drawList->AddRectFilled(start, end,
            // mPalette[(int)(focused ? PaletteIndex::CurrentLineFill :
            // PaletteIndex::CurrentLineFillInactive)]); drawList->AddRect(start, end,
            ...
            // 			ImVec2 cend(textScreenPos.x + cx + (mOverwrite ? mCharAdvance.x
            // : 1.0f), lineStartScreenPos.y + mCharAdvance.y);
...
void TextEditor::MoveEnd(bool aSelect)
{
    auto oldPos = mState.mCursorPosition;
    SetCursorPosition(
        Coordinates(mState.mCursorPosition.mLine, (int)mLines[oldPos.mLine].size()));
    ...
    if (mCheckComments) {
        ...
        for (auto currentCoord = Coordinates(0, 0); currentCoord < end;
            ...
            if (!line.empty()) {
                ...
                if (currentCoord.mColumn == (int)(line.size() - 1) &&
