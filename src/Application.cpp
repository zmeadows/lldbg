...
// NOTE: remember this is updated once per frame and currently variables are
// never removed
...
static std::pair<fs::path, size_t>
get_stop_location_from_frame(lldb::SBFrame frame) {
  ...
  return {fs::path(directory) / fs::path(filename),
          (int)frame.GetLineEntry().GetLine()};
}

static std::pair<fs::path, int>
resolve_breakpoint(lldb::SBBreakpointLocation location) {
...
static void set_thread_indices(UserInterface &ui, uint32_t thread_index,
...
static bool process_is_stopped(lldb::SBProcess &process) {
  ...
  return process.IsValid() &&
         (state == lldb::eStateStopped || state == lldb::eStateUnloaded);
}
...
static void stop_process(lldb::SBProcess &process) {
  ...
  if (err.Fail()) {
    LOG(Error)
        << "Failed to stop the process, encountered the following error: "
        << err.GetCString();
    return;
  }
}
...
static void continue_process(lldb::SBProcess& process)
{
    ...
    if (err.Fail()) {
        LOG(Error) << "Failed to continue the process, encountered the following
error: "
                   << err.GetCString();
        return;
    }
...
static void kill_process(lldb::SBProcess &process) {
  ...
  if (err.Fail()) {
    LOG(Error)
        << "Failed to kill the process, encountered the following error: \n\t"
        << err.GetCString();
    return;
  }
}
...
// A convenience struct for extracting pertinent display information from an
// lldb::SBFrame
struct StackFrame {
...
private:
  StackFrame(FileHandle _file_handle, int _line, int _column,
             std::string &&_function_name)
      : file_handle(_file_handle), function_name(std::move(_function_name)),
...
public:
  static std::optional<StackFrame> create(lldb::SBFrame frame) {
    ...
    if (auto handle = FileHandle::create(directory / filename);
        handle.has_value()) {
      ...
      LOG(Warning)
          << "Filepath corresponding to lldb stack frame doesn't exist: "
          << directory / filename;
      return {};
    }
  }
...
static bool FileTreeNode(const char *label) {
  ...
  const auto label_location =
      ImVec2(pos.x + button_sz + g.Style.ItemInnerSpacing.x,
...
static bool Splitter(const char *name, bool split_vertically, float thickness,
  ...
  ImRect bb;
  ImVec2 cursor_pos =
      (reset_cursor_pos ? ImVec2(0, 0) : window->DC.CursorPos) + offset;
  bb.Min = cursor_pos +
...
static void draw_open_files(Application &app) {
  ...
  app.open_files.for_each_open_file([&](FileHandle handle, bool is_focused) {
    ...
    // for example when the user clicks an entry in the stack trace or file
    // explorer
    ...
    if (ImGui::BeginTabItem(handle.filename().c_str(), &keep_tab_open,
                            tab_flags)) {
      ...
      if (clicked_line.has_value()) {
        ...
        if (focus_handle.has_value()) {
          ...
          for (uint32_t i = 0; i < num_breakpoints; i++) {
            ...
            auto [location_filepath, location_line] =
                resolve_breakpoint(location);
            if (location_filepath == filepath &&
                location_line == *clicked_line) {
...
static void manually_open_and_or_focus_file(UserInterface &ui,
                                            OpenFiles &open_files,
                                            FileHandle handle,
                                            size_t linum = 0) {
  if (auto focus = open_files.focus();
      focus.has_value() && (*focus == handle)) {
...
static void manually_open_and_or_focus_file(UserInterface &ui,
  ...
  if (auto handle = FileHandle::create(filepath); handle.has_value()) {
    ...
    LOG(Warning)
        << "Failed to switch focus to file because it could not be located: "
        << filepath;
  }
}

static void draw_file_browser(Application &app, FileBrowserNode *node_to_draw,
                              size_t depth) {
  assert(node_to_draw);
  ...
  if (node_to_draw->is_directory()) {
    ...
    if (ImGui::Selectable(node_to_draw->filename())) {
      manually_open_and_or_focus_file(app.ui, app.open_files,
                                      node_to_draw->filepath());
    }
  }
}

static lldb::SBCommandReturnObject
run_lldb_command(lldb::SBDebugger &debugger, LLDBCommandLine &cmdline,
                 const lldb::SBListener &listener, const char *command,
...
lldb::SBCommandReturnObject run_lldb_command(Application &app,
  ...
  return run_lldb_command(app.debugger, app.cmdline, app.listener, command,
                          hide_from_history);
}

static void draw_control_bar(lldb::SBDebugger &debugger,
                             LLDBCommandLine &cmdline,
                             const lldb::SBListener &listener,
                             const UserInterface &ui) {
  auto target = find_target(debugger);
  if (target.has_value()) {
    // TODO: show rightmost chunk of path in case it is too long to fit on
    // screen
    ...
    if (target_directory != nullptr && target_filename != nullptr) {
      target_description.format("Target: {}/{}", target_directory,
                                target_filename);
    } else {
  ...
  if (!target.has_value()) {
    ...
    if (ImGuiFileDialog::Instance()->Display("ChooseFileDlgKey")) {
      if (ImGuiFileDialog::Instance()->IsOk()) {
        ...
        run_lldb_command(debugger, cmdline, listener,
                         std::string("target create " + filePathName).c_str());
      }
  ...
  } else if (const auto [finished, _] = process_is_finished(*process);
             finished) {
    if (ImGui::Button("restart")) {
      run_lldb_command(debugger, cmdline, listener, "run");
    }
...
static void draw_file_viewer(Application &app) {
  ImGui::BeginChild("FileViewer", ImVec2(app.ui.file_viewer_width,
                                         app.ui.file_viewer_height));
...
static void draw_console(Application &app) {
  ...
  if (ImGui::BeginTabBar("##ConsoleLogTabs", ImGuiTabBarFlags_None)) {
    if (ImGui::BeginTabItem("console")) {
      ...
      // a command was run last frame, so that the user can immediately see the
      // output.
    ...
    if (ImGui::BeginTabItem("log")) {
      ...
      Logger::get_instance()->for_each_message(
          [](const LogMessage &entry) -> void {
            ...
            case LogLevel::Debug: {
              ImGui::TextColored(ImVec4(52.f / 255.f, 56.f / 255.f,
                                        176.f / 255.f, 255.f / 255.f),
                                 "[DEBUG]");
              break;
            }
            case LogLevel::Info: {
            ...
            case LogLevel::Error: {
              ImGui::TextColored(ImVec4(212.f / 255.f, 67.f / 255.f,
                                        67.f / 255.f, 255.f / 255.f),
                                 "[ERROR]");
              break;
            }
            }
    ...
    // TODO: these quantities need to be reset whenever the target is reset or
...
static void draw_threads(UserInterface &ui,
                         std::optional<lldb::SBProcess> process,
                         float stack_height) {
  ImGui::BeginChild("#ThreadsChild",
                    // ImVec2(ui.window_width - ui.file_browser_width -
                    // ui.file_viewer_width, stack_height));
    ...
    if (ImGui::BeginTabItem("stack trace")) {
      if (process.has_value() && process_is_stopped(*process)) {
        ...
        lldb::SBThread viewed_thread =
            process->GetThreadAtIndex(ui.viewed_thread_index);
        ...
        for (uint32_t i = 0; i < viewed_thread.GetNumFrames(); i++) {
          ...
          if (ImGui::Selectable(frame->function_name.c_str(),
                                i == ui.viewed_frame_index,
                                ImGuiSelectableFlags_SpanAllColumns)) {
            const auto [_, linum] =
                get_stop_location_from_frame(viewed_thread.GetFrameAtIndex(i));
...
static void draw_locals_and_registers(UserInterface &ui,
                                      std::optional<lldb::SBProcess> process,
                                      float stack_height) {
    ...
    if (ImGui::BeginTabItem("locals")) {
      if (process.has_value() && process_is_stopped(*process)) {
        ...
        lldb::SBThread viewed_thread =
            process->GetThreadAtIndex(ui.viewed_thread_index);
        lldb::SBFrame frame =
            viewed_thread.GetFrameAtIndex(ui.viewed_frame_index);
      ...
      if (process.has_value() && process_is_stopped(*process)) {
        ...
        if (viewed_thread.IsValid() && frame.IsValid()) {
          ...
          for (uint32_t i = 0; i < register_collections.GetSize(); i++) {
            ...
            if (!collection_name) {
              LOG(Warning)
                  << "Skipping over invalid/un-named register collection";
              continue;
            }
...
static void
draw_breakpoints_and_watchpoints(UserInterface &ui, OpenFiles &open_files,
    ...
    if (ImGui::BeginTabItem("breakpoints")) {
      ...
      if (target.has_value()) {
        ...
        for (uint32_t i = 0; i < nbreakpoints; i++) {
          ...
          if (filename == nullptr || directory == nullptr) {
            LOG(Error)
                << "invalid/unspecified filepath encountered for breakpoint";
            continue;
          }
          ...
          if (ImGui::Selectable(filename, i == ui.viewed_breakpoint_index,
            ...
            fs::path breakpoint_filepath =
                fs::path(directory) / fs::path(filename);
...
__attribute__((flatten)) static void draw(Application &app) {
  ...
  ImGui::SetNextWindowSize(ImVec2(ui.window_width, ui.window_height),
                           ImGuiCond_Always);
...

  {
    ...
    ImGui::BeginGroup();
    ImGui::BeginChild("ControlBarAndFileBrowser",
                      ImVec2(ui.file_browser_width, 0));
...

  {
    Splitter("##S2", false, 3.0f, &ui.file_viewer_height, &ui.console_height,
             0.1 * ui.window_height, 0.1 * ui.window_height,
             ui.file_viewer_width, false, ImVec2(0, 0));
  ...
  {
    float stack_height = (ui.window_height * ui.dpi_scale -
  ...
  // Handle highlighted lines
  {
    ...
    if (linum.has_value() &&
        (*(app.file_viewer.get_highlight_line()) != (int)*linum))
...
static void handle_lldb_events(lldb::SBDebugger &debugger,
                               lldb::SBListener &listener, UserInterface &ui,
                               OpenFiles &open_files, FileViewer &file_viewer) {
  lldb::SBEvent event;
  ...
  while (true) {
    ...
    if (target.has_value() &&
        event.BroadcasterMatchesRef(target->GetBroadcaster())) {
      LOG(Debug) << "Found target event";
      file_viewer.synchronize_breakpoint_cache(*target);
    } else if (process.has_value() &&
               event.BroadcasterMatchesRef(process->GetBroadcaster())) {
      const lldb::StateType new_state =
      ...
      // For now we find the first (if any) stopped thread and construct a
      // StopInfo.
        ...
        for (uint32_t i = 0; i < nthreads; i++) {
          ...
          case lldb::eStopReasonBreakpoint: {
            ...
            if (th.GetStopReasonDataCount() != 2)
              LOG(Debug) << "Stop Reason Data Count"
                         << th.GetStopReasonDataCount();
...
static void tick(Application &app) {
  handle_lldb_events(app.debugger, app.listener, app.ui, app.open_files,
                     app.file_viewer);
...
static void update_window_dimensions(UserInterface &ui) {
  ...
  float dpi_scale_factor = (float)std::max(window_width, window_height) /
                           (float)std::max(new_width, new_height);

  ui.window_resized_last_frame = new_width != ui.window_width ||
                                 new_height != ui.window_height ||
                                 dpi_scale_factor != ui.dpi_scale;
...
std::optional<UserInterface> UserInterface::init(void) {
  ...
  // TODO: use function to choose initial window resolution based on display
  // resolution
...
Application::Application(const UserInterface &ui_,
                         std::optional<fs::path> workdir)
    : debugger(lldb::SBDebugger::Create()), listener(debugger.GetListener()),
      cmdline(debugger), _stdout(StreamBuffer::StreamSource::StdOut),
      _stderr(StreamBuffer::StreamSource::StdErr),
      file_browser(FileBrowserNode::create(workdir)), ui(ui_) {}

Application::~Application() {
  if (auto process = find_process(this->debugger);
      process.has_value() && process->IsValid()) {
    LOG(Warning) << "Found active process while closing Application.";
    kill_process(*process);
  }

  if (auto target = find_target(this->debugger);
