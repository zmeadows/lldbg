...
template <class T>
void shared_ptr_pointer<T>::on_zero_shared()
{
    delete data_;
}

template <class T>
class shared_ptr_emplace : public shared_count {
    T data_;
...
public:
    ...
    template <class A0>
    shared_ptr_emplace(A0& a0) : data_(a0)
    {
    }
    ...
    shared_ptr_emplace(A0& a0, A1& a1, A2& a2, A3& a3, A4& a4) : data_(a0, a1, a2, a3, a4)
    {
    }
...
template <class T>
void shared_ptr_emplace<T>::on_zero_shared()
{
}
...
public:
    ...
    template <class Y>
    explicit SharingPtr(Y* p);
    template <class Y>
    explicit SharingPtr(Y* p, imp::shared_count* ctrl_block);
    template <class Y>
    ...
    template <class Y>
    SharingPtr& operator=(const SharingPtr<Y>& r);
...
template <class T>
inline SharingPtr<T>::SharingPtr(std::nullptr_t) : ptr_(nullptr), cntrl_(nullptr)
{
}
...
template <class T>
template <class Y>
SharingPtr<T>::SharingPtr(Y* p, imp::shared_count* cntrl_block) : ptr_(p), cntrl_(cntrl_block)
{
}
...
template <class T>
inline SharingPtr<T>::SharingPtr(const SharingPtr& r) : ptr_(r.ptr_), cntrl_(r.cntrl_)
{
    if (cntrl_) cntrl_->add_shared();
}
...
template <class T>
template <class Y>
inline SharingPtr<T>::SharingPtr(const SharingPtr<Y>& r) : ptr_(r.ptr_), cntrl_(r.cntrl_)
{
    if (cntrl_) cntrl_->add_shared();
}
...
{
    if (cntrl_) cntrl_->release_shared();
}
...
template <class T>
inline void SharingPtr<T>::swap(SharingPtr& r)
{
...
template <class T>
inline void SharingPtr<T>::reset()
{
    SharingPtr().swap(*this);
}

template <class T>
inline void SharingPtr<T>::reset(std::nullptr_t /* p */)
{
    reset();
}
...
template <class T>
template <class Y>
inline void SharingPtr<T>::reset(Y* p)
{
    SharingPtr(p).swap(*this);
}

template <class T>
SharingPtr<T> SharingPtr<T>::make_shared()
{
...
template <class T>
template <class A0, class A1, class A2, class A3, class A4>
SharingPtr<T> SharingPtr<T>::make_shared(A0& a0, A1& a1, A2& a2, A3& a3, A4& a4)
{
    typedef imp::shared_ptr_emplace<T> CntrlBlk;
    SharingPtr<T> r;
...
template <class T>
inline SharingPtr<T> make_shared()
{
    return SharingPtr<T>::make_shared();
}

template <class T, class A0>
inline SharingPtr<T> make_shared(A0& a0)
{
...
template <class T>
inline void swap(SharingPtr<T>& __x, SharingPtr<T>& __y)
{
    __x.swap(__y);
}
...
template <class T>
class LoggingSharingPtr : public SharingPtr<T> {
...
public:
    ...
    template <class Y>
    LoggingSharingPtr(Y* p, Callback cb, void* baton) : base(p), cb_(cb), baton_(baton)
    {
        if (cb_) cb_(baton_, *this, false);
    }
    ...
    ~LoggingSharingPtr()
    {
        if (cb_) cb_(baton_, *this, true);
    }

    LoggingSharingPtr(const LoggingSharingPtr& p) : base(p), cb_(p.cb_), baton_(p.baton_)
    {
        if (cb_) cb_(baton_, *this, false);
    }
...
template <class T>
void lldb_private::ReferenceCountedBase<T>::add_shared()
{
#ifdef _MSC_VER
...
template <class T>
void ReferenceCountedBaseVirtual<T>::on_zero_shared()
{
}
...
public:
    ...
    IntrusiveSharingPtr(const IntrusiveSharingPtr& rhs) : ptr_(rhs.ptr_) { add_shared(); }
